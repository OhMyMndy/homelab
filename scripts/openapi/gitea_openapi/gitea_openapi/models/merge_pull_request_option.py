...
class MergePullRequestOption(BaseModel):
    ...
    def do_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['merge', 'rebase', 'rebase-merge', 'squash', 'fast-forward-only', 'manually-merged']):
            raise ValueError("must be one of enum values ('merge', 'rebase', 'rebase-merge', 'squash', 'fast-forward-only', 'manually-merged')")
        return value
