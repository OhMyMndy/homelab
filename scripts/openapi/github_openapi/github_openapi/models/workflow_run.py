...
class WorkflowRun(BaseModel):
    ...
    display_title: Optional[StrictStr] = Field(default=None, description="The event-specific title associated with the run or the run-name if set, or the value of `run-name` if it is set in the workflow.")
    __properties: ClassVar[List[str]] = ["actor", "artifacts_url", "cancel_url", "check_suite_id", "check_suite_node_id", "check_suite_url", "conclusion", "created_at", "event", "head_branch", "head_commit", "head_repository", "head_sha", "html_url", "id", "jobs_url", "logs_url", "name", "node_id", "path", "previous_attempt_url", "pull_requests", "referenced_workflows", "repository", "rerun_url", "run_attempt", "run_number", "run_started_at", "status", "triggering_actor", "updated_at", "url", "workflow_id", "workflow_url", "display_title"]
    ...
    def conclusion_validate_enum(cls, value):
        ...
        if value not in set(['action_required', 'cancelled', 'failure', 'neutral', 'skipped', 'stale', 'success', 'timed_out', 'startup_failure']):
            raise ValueError("must be one of enum values ('action_required', 'cancelled', 'failure', 'neutral', 'skipped', 'stale', 'success', 'timed_out', 'startup_failure')")
        return value

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['requested', 'in_progress', 'completed', 'queued', 'pending', 'waiting']):
            raise ValueError("must be one of enum values ('requested', 'in_progress', 'completed', 'queued', 'pending', 'waiting')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "head_repository": RepositoryLite.from_dict(obj["head_repository"]) if obj.get("head_repository") is not None else None,
            ...
            "repository": RepositoryLite.from_dict(obj["repository"]) if obj.get("repository") is not None else None,
