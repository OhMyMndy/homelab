...
class WebhookMergeGroupDestroyed(BaseModel):
    ...
    """ # noqa: E501
    action: StrictStr
    reason: Optional[StrictStr] = Field(default=None, description="Explains why the merge group is being destroyed. The group could have been merged, removed from the queue (dequeued), or invalidated by an earlier queue entry being dequeued (invalidated).")
    ...
    __properties: ClassVar[List[str]] = ["action", "reason", "installation", "merge_group", "organization", "repository", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['destroyed']):
            raise ValueError("must be one of enum values ('destroyed')")
        return value
    ...
    def reason_validate_enum(cls, value):
        ...
        if value not in set(['merged', 'invalidated', 'dequeued']):
            raise ValueError("must be one of enum values ('merged', 'invalidated', 'dequeued')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of merge_group
        if self.merge_group:
            _dict['merge_group'] = self.merge_group.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "merge_group": MergeGroup.from_dict(obj["merge_group"]) if obj.get("merge_group") is not None else None,
