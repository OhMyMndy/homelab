...
class WebhookInstallationNewPermissionsAccepted(BaseModel):
    ...
    repositories: Optional[List[WebhooksRepositoriesInner]] = Field(default=None, description="An array of repository objects that the installation can access.")
    repository: Optional[RepositoryWebhooks] = None
    requester: Optional[Any] = None
    sender: SimpleUser
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "organization", "repositories", "repository", "requester", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['new_permissions_accepted']):
            raise ValueError("must be one of enum values ('new_permissions_accepted')")
        return value
