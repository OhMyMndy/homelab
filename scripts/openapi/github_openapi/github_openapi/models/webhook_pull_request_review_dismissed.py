...
class WebhookPullRequestReviewDismissed(BaseModel):
    ...
    sender: SimpleUser
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "organization", "pull_request", "repository", "review", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['dismissed']):
            raise ValueError("must be one of enum values ('dismissed')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "pull_request": SimplePullRequest.from_dict(obj["pull_request"]) if obj.get("pull_request") is not None else None,
            "repository": RepositoryWebhooks.from_dict(obj["repository"]) if obj.get("repository") is not None else None,
            "review": WebhookPullRequestReviewDismissedReview.from_dict(obj["review"]) if obj.get("review") is not None else None,
