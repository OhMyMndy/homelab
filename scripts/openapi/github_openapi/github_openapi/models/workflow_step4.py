...
class WorkflowStep4(BaseModel):
    ...
    def conclusion_validate_enum(cls, value):
        ...
        if value not in set(['failure', 'skipped', 'success', 'cancelled']):
            raise ValueError("must be one of enum values ('failure', 'skipped', 'success', 'cancelled')")
        return value

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['completed', 'in_progress', 'queued', 'pending', 'waiting']):
            raise ValueError("must be one of enum values ('completed', 'in_progress', 'queued', 'pending', 'waiting')")
        return value
