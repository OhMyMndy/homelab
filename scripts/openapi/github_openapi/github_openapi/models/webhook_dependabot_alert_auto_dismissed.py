...
class WebhookDependabotAlertAutoDismissed(BaseModel):
    ...
    sender: SimpleUser
    __properties: ClassVar[List[str]] = ["action", "alert", "installation", "organization", "enterprise", "repository", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['auto_dismissed']):
            raise ValueError("must be one of enum values ('auto_dismissed')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "alert": DependabotAlert.from_dict(obj["alert"]) if obj.get("alert") is not None else None,
