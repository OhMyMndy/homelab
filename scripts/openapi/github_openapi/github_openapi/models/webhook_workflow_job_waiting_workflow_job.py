...
class WebhookWorkflowJobWaitingWorkflowJob(BaseModel):
    """
    ...
    run_id: Union[StrictFloat, StrictInt]
    run_url: StrictStr
    runner_group_id: Optional[StrictInt]
    runner_group_name: Optional[StrictStr]
    runner_id: Optional[StrictInt]
    ...
    status: StrictStr
    steps: List[WorkflowStep4]
    url: StrictStr
    __properties: ClassVar[List[str]] = ["check_run_url", "completed_at", "conclusion", "created_at", "head_sha", "html_url", "id", "labels", "name", "node_id", "run_attempt", "run_id", "run_url", "runner_group_id", "runner_group_name", "runner_id", "runner_name", "started_at", "head_branch", "workflow_name", "status", "steps", "url"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['queued', 'in_progress', 'completed', 'waiting']):
            raise ValueError("must be one of enum values ('queued', 'in_progress', 'completed', 'waiting')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "steps": [WorkflowStep4.from_dict(_item) for _item in obj["steps"]] if obj.get("steps") is not None else None,
            "url": obj.get("url")
        })
        return _obj
