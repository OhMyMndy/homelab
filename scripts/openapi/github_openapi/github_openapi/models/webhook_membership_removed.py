...
class WebhookMembershipRemoved(BaseModel):
    ...
    member: Optional[WebhooksUser]
    organization: OrganizationSimpleWebhooks
    repository: Optional[RepositoryWebhooks] = None
    scope: StrictStr = Field(description="The scope of the membership. Currently, can only be `team`.")
    sender: Optional[User2]
    team: WebhooksTeam
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "member", "organization", "repository", "scope", "sender", "team"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['removed']):
            raise ValueError("must be one of enum values ('removed')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "member": WebhooksUser.from_dict(obj["member"]) if obj.get("member") is not None else None,
            ...
            "scope": obj.get("scope"),
            "sender": User2.from_dict(obj["sender"]) if obj.get("sender") is not None else None,
            "team": WebhooksTeam.from_dict(obj["team"]) if obj.get("team") is not None else None
        })
        return _obj
