...
class WebhookSponsorshipPendingTierChange(BaseModel):
    ...
    effective_date: Optional[StrictStr] = Field(default=None, description="The `pending_cancellation` and `pending_tier_change` event types will include the date the cancellation or tier change will take effect.")
    ...
    __properties: ClassVar[List[str]] = ["action", "changes", "effective_date", "enterprise", "installation", "organization", "repository", "sender", "sponsorship"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pending_tier_change']):
            raise ValueError("must be one of enum values ('pending_tier_change')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of sponsorship
        if self.sponsorship:
            _dict['sponsorship'] = self.sponsorship.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "changes": WebhooksChanges8.from_dict(obj["changes"]) if obj.get("changes") is not None else None,
            ...
            "sponsorship": WebhooksSponsorship.from_dict(obj["sponsorship"]) if obj.get("sponsorship") is not None else None
        })
        return _obj
