...
class WebhookPullRequestReviewRequestRemovedOneOf(BaseModel):
    """
    ...
    requested_reviewer: Optional[User2]
    sender: SimpleUser
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "number", "organization", "pull_request", "repository", "requested_reviewer", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['review_request_removed']):
            raise ValueError("must be one of enum values ('review_request_removed')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "pull_request": PullRequest6.from_dict(obj["pull_request"]) if obj.get("pull_request") is not None else None,
            "repository": RepositoryWebhooks.from_dict(obj["repository"]) if obj.get("repository") is not None else None,
            "requested_reviewer": User2.from_dict(obj["requested_reviewer"]) if obj.get("requested_reviewer") is not None else None,
