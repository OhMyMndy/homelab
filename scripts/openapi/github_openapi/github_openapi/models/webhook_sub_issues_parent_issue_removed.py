...
class WebhookSubIssuesParentIssueRemoved(BaseModel):
    ...
    """ # noqa: E501
    action: StrictStr
    parent_issue_id: Union[StrictFloat, StrictInt] = Field(description="The ID of the parent issue.")
    parent_issue: Issue
    parent_issue_repo: Repository
    sub_issue_id: Union[StrictFloat, StrictInt] = Field(description="The ID of the sub-issue.")
    sub_issue: Issue
    ...
    __properties: ClassVar[List[str]] = ["action", "parent_issue_id", "parent_issue", "parent_issue_repo", "sub_issue_id", "sub_issue", "installation", "organization", "repository", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['parent_issue_removed']):
            raise ValueError("must be one of enum values ('parent_issue_removed')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of parent_issue
        if self.parent_issue:
            _dict['parent_issue'] = self.parent_issue.to_dict()
        # override the default output from pydantic by calling `to_dict()` of parent_issue_repo
        if self.parent_issue_repo:
            _dict['parent_issue_repo'] = self.parent_issue_repo.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "parent_issue": Issue.from_dict(obj["parent_issue"]) if obj.get("parent_issue") is not None else None,
            "parent_issue_repo": Repository.from_dict(obj["parent_issue_repo"]) if obj.get("parent_issue_repo") is not None else None,
            "sub_issue_id": obj.get("sub_issue_id"),
            "sub_issue": Issue.from_dict(obj["sub_issue"]) if obj.get("sub_issue") is not None else None,
