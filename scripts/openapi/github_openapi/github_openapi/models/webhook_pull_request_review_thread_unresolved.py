...
class WebhookPullRequestReviewThreadUnresolved(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "organization", "pull_request", "repository", "sender", "thread"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['unresolved']):
            raise ValueError("must be one of enum values ('unresolved')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "pull_request": SimplePullRequest4.from_dict(obj["pull_request"]) if obj.get("pull_request") is not None else None,
            ...
            "thread": WebhookPullRequestReviewThreadUnresolvedThread.from_dict(obj["thread"]) if obj.get("thread") is not None else None
        })
        return _obj
