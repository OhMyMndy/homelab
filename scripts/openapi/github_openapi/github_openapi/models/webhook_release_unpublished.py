...
class WebhookReleaseUnpublished(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "organization", "release", "repository", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['unpublished']):
            raise ValueError("must be one of enum values ('unpublished')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "release": WebhooksRelease1.from_dict(obj["release"]) if obj.get("release") is not None else None,
