...
class WebhookPullRequestDequeued(BaseModel):
    ...
    sender: SimpleUser
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "number", "organization", "pull_request", "reason", "repository", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['dequeued']):
            raise ValueError("must be one of enum values ('dequeued')")
        return value

    @field_validator('reason')
    def reason_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['UNKNOWN_REMOVAL_REASON', 'MANUAL', 'MERGE', 'MERGE_CONFLICT', 'CI_FAILURE', 'CI_TIMEOUT', 'ALREADY_MERGED', 'QUEUE_CLEARED', 'ROLL_BACK', 'BRANCH_PROTECTIONS', 'GIT_TREE_INVALID', 'INVALID_MERGE_COMMIT']):
            raise ValueError("must be one of enum values ('UNKNOWN_REMOVAL_REASON', 'MANUAL', 'MERGE', 'MERGE_CONFLICT', 'CI_FAILURE', 'CI_TIMEOUT', 'ALREADY_MERGED', 'QUEUE_CLEARED', 'ROLL_BACK', 'BRANCH_PROTECTIONS', 'GIT_TREE_INVALID', 'INVALID_MERGE_COMMIT')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "pull_request": PullRequest3.from_dict(obj["pull_request"]) if obj.get("pull_request") is not None else None,
