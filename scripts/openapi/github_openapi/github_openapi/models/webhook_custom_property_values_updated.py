...
class WebhookCustomPropertyValuesUpdated(BaseModel):
    ...
    new_property_values: List[CustomPropertyValue] = Field(description="The new custom property values for the repository.")
    old_property_values: List[CustomPropertyValue] = Field(description="The old custom property values for the repository.")
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "repository", "organization", "sender", "new_property_values", "old_property_values"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['updated']):
            raise ValueError("must be one of enum values ('updated')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in new_property_values (list)
        _items = []
        if self.new_property_values:
            for _item_new_property_values in self.new_property_values:
                if _item_new_property_values:
                    _items.append(_item_new_property_values.to_dict())
            _dict['new_property_values'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in old_property_values (list)
        _items = []
        if self.old_property_values:
            for _item_old_property_values in self.old_property_values:
                if _item_old_property_values:
                    _items.append(_item_old_property_values.to_dict())
            _dict['old_property_values'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "new_property_values": [CustomPropertyValue.from_dict(_item) for _item in obj["new_property_values"]] if obj.get("new_property_values") is not None else None,
            "old_property_values": [CustomPropertyValue.from_dict(_item) for _item in obj["old_property_values"]] if obj.get("old_property_values") is not None else None
        })
        return _obj
