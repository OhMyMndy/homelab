...
class WebhookPullRequestUnlabeled(BaseModel):
    ...
    label: Optional[WebhooksLabel] = None
    ...
    sender: SimpleUser
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "label", "number", "organization", "pull_request", "repository", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['unlabeled']):
            raise ValueError("must be one of enum values ('unlabeled')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "label": WebhooksLabel.from_dict(obj["label"]) if obj.get("label") is not None else None,
            "number": obj.get("number"),
            "organization": OrganizationSimpleWebhooks.from_dict(obj["organization"]) if obj.get("organization") is not None else None,
            "pull_request": PullRequest12.from_dict(obj["pull_request"]) if obj.get("pull_request") is not None else None,
