...
class WebhookTeamRemovedFromRepository(BaseModel):
    ...
    sender: SimpleUser
    team: WebhooksTeam1
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "organization", "repository", "sender", "team"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['removed_from_repository']):
            raise ValueError("must be one of enum values ('removed_from_repository')")
        return value
