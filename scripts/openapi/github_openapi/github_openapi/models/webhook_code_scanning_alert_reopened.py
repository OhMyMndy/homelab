...
class WebhookCodeScanningAlertReopened(BaseModel):
    ...
    """ # noqa: E501
    action: StrictStr
    alert: Optional[WebhookCodeScanningAlertReopenedAlert]
    commit_oid: Optional[StrictStr] = Field(description="The commit SHA of the code scanning alert. When the action is `reopened_by_user` or `closed_by_user`, the event was triggered by the `sender` and this value will be empty.")
    ...
    ref: Optional[StrictStr] = Field(description="The Git reference of the code scanning alert. When the action is `reopened_by_user` or `closed_by_user`, the event was triggered by the `sender` and this value will be empty.")
    repository: RepositoryWebhooks
    sender: SimpleUser
    __properties: ClassVar[List[str]] = ["action", "alert", "commit_oid", "enterprise", "installation", "organization", "ref", "repository", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['reopened']):
            raise ValueError("must be one of enum values ('reopened')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "alert": WebhookCodeScanningAlertReopenedAlert.from_dict(obj["alert"]) if obj.get("alert") is not None else None,
