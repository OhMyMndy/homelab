...
class WebhookSecretScanningAlertPubliclyLeaked(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["action", "alert", "enterprise", "installation", "organization", "repository", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['publicly_leaked']):
            raise ValueError("must be one of enum values ('publicly_leaked')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "alert": SecretScanningAlertWebhook.from_dict(obj["alert"]) if obj.get("alert") is not None else None,
