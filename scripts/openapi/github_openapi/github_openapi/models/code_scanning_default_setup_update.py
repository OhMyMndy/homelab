...
class CodeScanningDefaultSetupUpdate(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["state", "runner_type", "runner_label", "query_suite", "languages"]
    ...
    def languages_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['c-cpp', 'csharp', 'go', 'java-kotlin', 'javascript-typescript', 'python', 'ruby', 'swift']):
                raise ValueError("each list item must be one of ('c-cpp', 'csharp', 'go', 'java-kotlin', 'javascript-typescript', 'python', 'ruby', 'swift')")
        return value
