...
class MaxFileSize(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr
    parameters: Optional[MaxFileSizeParameters] = None
    __properties: ClassVar[List[str]] = ["type", "parameters"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['max_file_size']):
            raise ValueError("must be one of enum values ('max_file_size')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "parameters": MaxFileSizeParameters.from_dict(obj["parameters"]) if obj.get("parameters") is not None else None
        })
        return _obj
