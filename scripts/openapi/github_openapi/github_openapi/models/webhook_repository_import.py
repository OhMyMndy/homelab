...
class WebhookRepositoryImport(BaseModel):
    ...
    sender: SimpleUser
    status: StrictStr
    __properties: ClassVar[List[str]] = ["enterprise", "installation", "organization", "repository", "sender", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['success', 'cancelled', 'failure']):
            raise ValueError("must be one of enum values ('success', 'cancelled', 'failure')")
        return value
