...
class WebhookRepositoryAdvisoryReported(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["action", "enterprise", "installation", "organization", "repository", "repository_advisory", "sender"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['reported']):
            raise ValueError("must be one of enum values ('reported')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of repository_advisory
        if self.repository_advisory:
            _dict['repository_advisory'] = self.repository_advisory.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "repository_advisory": RepositoryAdvisory.from_dict(obj["repository_advisory"]) if obj.get("repository_advisory") is not None else None,
