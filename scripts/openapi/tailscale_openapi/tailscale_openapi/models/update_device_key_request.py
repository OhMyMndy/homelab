...
class UpdateDeviceKeyRequest(BaseModel):
    """
    UpdateDeviceKeyRequest
    """ # noqa: E501
    key_expiry_disabled: StrictBool = Field(description="- If `true`, disable the device's key expiry. The original key expiry time is still maintained. Upon re-enabling, the key will expire at that original time. - If `false`, enable the device's key expiry. Sets the key to expire at the original expiry time prior to disabling. The key may already have expired. In that case, the device must be re-authenticated. ", alias="keyExpiryDisabled")
    __properties: ClassVar[List[str]] = ["keyExpiryDisabled"]
