...
class CreateDeviceInvitesRequestInner(BaseModel):
    """
    CreateDeviceInvitesRequestInner
    """ # noqa: E501
    multi_use: Optional[StrictBool] = Field(default=None, description="Whether the invite can be accepted more than once. When set to `true`, it results in an invite that can be accepted up to 1,000 times. ", alias="multiUse")
    allow_exit_node: Optional[StrictBool] = Field(default=None, description="Whether the invited user can use the device as an exit node when it advertises as one. ", alias="allowExitNode")
    email: Optional[StrictStr] = Field(default=None, description="The email to send the created invite to. If not set, the endpoint generates and returns an invite URL (but doesn't send it out). ")
    __properties: ClassVar[List[str]] = ["multiUse", "allowExitNode", "email"]
