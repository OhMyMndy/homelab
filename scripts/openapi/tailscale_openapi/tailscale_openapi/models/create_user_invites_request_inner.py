...
class CreateUserInvitesRequestInner(BaseModel):
    """
    CreateUserInvitesRequestInner
    """ # noqa: E501
    role: Optional[StrictStr] = Field(default='member', description="Optionally specifies a user role to assign the invited user. ")
    email: Optional[StrictStr] = Field(default=None, description="Optionally specifies the email to send the created invite. If not set, the endpoint generates and returns an invite URL, but does not email it out. ")
    __properties: ClassVar[List[str]] = ["role", "email"]
