    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.device_challenges:
            for _item_device_challenges in self.device_challenges:
                if _item_device_challenges:
                    _items.append(_item_device_challenges.to_dict())
            _dict['device_challenges'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in configuration_stages (list)
        _items = []
        if self.configuration_stages:
            for _item_configuration_stages in self.configuration_stages:
                if _item_configuration_stages:
                    _items.append(_item_configuration_stages.to_dict())
